...
@section('Booking-Settings','active')
...
<div id="main" class="p-4 light-mode">
   ...
   <script>
      ...
      deleteModal.addEventListener('show.bs.modal', function (event) {
         ...
         var bookingsId = button.getAttribute('data-id');

         var form = document.getElementById('deleteForm');
         form.action = '/bookings/' + bookingsId;
      });
   </script>
   <style>
      .myshadow{
   ...
   <div class="d-flex justify-content-between align-items-center mb-4">
      <div>
         <h4 class="fw-bold text-primary">Booking Management</h4>
         <small class="text-muted">Manage all Booking</small>
      </div>
   ...
   <div class="card shadow-sm border-0">
      <div class="card-body">
         {{-- Search --}}
        <form method="GET" action="{{ route('bookingList.index') }}">
         ...
         <div class="table-responsive"  id="customerTable">
            <table class="table align-middle" >
               ...
               <tbody>
                  @forelse ($mybooking as $value)
                     <tr>
                        <td class="fw-semibold">{{ $value->customer->name }}</td>
                        <td>{{ $value->room->name }}</td>
                        <td>{{ $value->booking_date }}</td>
                        <td>{{ $value->check_in }}</td>
                        <td>{{ $value->check_out }}</td>
                        <td>{{ number_format($value->adults,0) }}</td>
                        <td>{{ $value->payment_method }}</td>
                        <td>${{ $value->total_price }}</td>
                        <td>
                           @if($value->customer && $value->customer->profile_photo)
                              <img src="{{ asset('assets/uploads/customers/'.$value->customer->profile_photo) }}"
                                    width="45" height="45"
